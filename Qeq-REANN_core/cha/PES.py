...
#            varene = output[i]
#            grad = torch.autograd.grad([varene], [cart], retain_graph=True, allow_unused=True)[0]
#            if grad is not None:
#                gradsList.append(grad)
#            else:
#                gradsList.append(torch.zeros_like(cart))
#    
#        if gradsList is not None:
#            grads = torch.stack(gradsList)
#            return output.detach(), -grads.detach()

#        gradsList=[]                
#        for i in range(output.shape[0]):
#          output[i].backward(retain_graph=True)
#          if cart.grad is not None:
#            gradsList.append(cart.grad.clone())
#            cart.grad.zero_()
#        grads=torch.stack(gradsList)
#        return output.detach(),-grads.detach()
