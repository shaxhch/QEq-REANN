...
void chPes::reann_chout(int chnumatoms, double *chcart, double *chenergy, double *chforce) {
  ...
  chcart_t=chcart_t.clone().detach().requires_grad_(true);
  ...
  for (int i=0;i<this->chnumatoms;i++){
    ...
    torch::autograd::backward({-chenergy_t[i]}, {}, true);
    auto chforce_pd = chcart_t.grad().to(torch::kDouble).data_ptr<double>();
    for (int j=0;j<this->chnumatoms;j++) {
      for (int k=0;k<3;k++) {
       chforce[i*this->chnumatoms*3+j*3+k]=chforce_pd[j*3+k];
      }
    }
    chcart_t.grad().zero_();
  }
